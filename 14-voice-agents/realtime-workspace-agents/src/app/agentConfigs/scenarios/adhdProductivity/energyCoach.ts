...
import { energyCoachPrompt } from './prompts';
...
// ---------------------------------------------------------------------------
// Energy Coach agent definition - ADHD/Embodied Work Productivity System
// ---------------------------------------------------------------------------

export const energyCoach = new RealtimeAgent({
  name: 'energyCoach',
  voice: 'sage',
  instructions: energyCoachPrompt,
...
async function handleToolCalls(
  ...
  while (true) {
    ...
    for (const toolCall of functionCalls) {
      ...
      if (addBreadcrumb) {
        addBreadcrumb(`[energyCoach] function call: ${fName}`, args);
      }
      if (addBreadcrumb) {
        addBreadcrumb(`[energyCoach] function call result: ${fName}`, toolRes);
      }
...
export const makeWorkspaceChanges = tool({
  ...
  description:
    'Make changes to productivity workspace - update energy levels, task progress, insights, or regulation notes.',
  parameters: {
    type: 'object',
    properties: {
      tabsToChange: {
      ...
      input: [
        {
          type: 'message',
          role: 'system',
          content: `You are an ADHD/productivity workspace assistant. Use the tools below to make changes to the productivity workspace based on the changes requested. 
          
          Default productivity tabs to create:
          - "Today's Plan" (markdown): Current day structure, energy levels, priority tasks
          - "Energy Patterns" (csv): Historical data on energy levels throughout day/week
          - "Task Bank" (csv): Ongoing list of tasks categorized by energy requirement
          - "Regulation Toolkit" (markdown): Personalized strategies for emotional regulation
          - "Wins & Insights" (markdown): Celebration log and pattern insights
          - "Body Doubling Log" (csv): Focus session records and what worked
